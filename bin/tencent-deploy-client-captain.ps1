...
Write-Host "🚀 部署团长端前端 (Client-Captain)..." -ForegroundColor Green
...
# 1. 构建
Write-Host "📦 构建团长端前端..." -ForegroundColor Green
Push-Location (Join-Path $ProjectRoot "client-captain")
npm install
...
$ArchivePath = Join-Path $ProjectRoot "client_captain_deploy.tar.gz"
if (Test-Path $ArchivePath) {
    Remove-Item $ArchivePath -Force
}

$DeployTmpDir = Join-Path $ProjectRoot "deploy_tmp_captain"
if (Test-Path $DeployTmpDir) {
    Remove-Item $DeployTmpDir -Recurse -Force
}
New-Item -ItemType Directory -Path $DeployTmpDir | Out-Null

$DistDir = Join-Path $ProjectRoot "client-captain\dist"
...
Write-Host "✅ 团长端前端部署完成！" -ForegroundColor Green
