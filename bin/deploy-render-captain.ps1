...
Write-Host "============================================================"
Write-Host "   🌐 部署 Captain 到 Render"
Write-Host "============================================================"
Write-Host ""

$ScriptDir = $PSScriptRoot
$ProjectRoot = Split-Path $ScriptDir -Parent
$EnvFile = Join-Path $ProjectRoot "client-captain\.env.render"
...
$DeployHook = [System.Environment]::GetEnvironmentVariable("RENDER_DEPLOY_HOOK_CAPTAIN")

# 检查 Deploy Hook URL 是否配置
if ([string]::IsNullOrWhiteSpace($DeployHook)) {
    Write-Host "[错误] RENDER_DEPLOY_HOOK_CAPTAIN 未配置"
    Write-Host ""
    Write-Host "请在 $EnvFile 中配置 Deploy Hook URL"
    Write-Host "获取方式: Render Dashboard > kiro-basket-client-captain > Settings > Deploy Hook"
    exit 1
}
