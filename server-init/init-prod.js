...
async function main() {
  console.log('═══════════════════════════════════════════════════');
  console.log('   🗄️  生产环境数据库初始化');
  console.log('═══════════════════════════════════════════════════');
  console.log('');
  console.log('⚠️  警告：这将操作生产数据库！');
  ...
  console.log('');
  console.log('🎯 环境: PROD');
  console.log('📁 配置: .env.prod');
  console.log('');

  // 检查配置文件
  const configPath = path.join(__dirname, '.env.prod');
  if (!fs.existsSync(configPath)) {
    console.error('❌ 配置文件不存在: .env.prod');
    rl.close();
    process.exit(1);
  }
  ...
  // 第二次确认
  const confirm2 = await question('确认使用以上配置初始化生产数据库？(输入 YES 继续): ');
  if (confirm2 !== 'YES') {
    console.log('❌ 操作已取消');
    rl.close();
    process.exit(0);
